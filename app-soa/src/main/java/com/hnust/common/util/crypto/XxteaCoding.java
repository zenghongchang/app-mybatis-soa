...
public class XxteaCoding {
	...
	public XxteaCoding(byte[] key) {
		this.key = XxteaCoding.makePassword(key);
	}
	...
	public byte[] encode(byte[] data) {
		...
		XxteaCoding.toByteArray(v, v.length, result);
		return result;
	}
	...
	public byte[] decode(byte[] data) {
		...
		byte[] result = new byte[n];
		XxteaCoding.toByteArray(v, v.length - 1, result);
		return result;
	}
