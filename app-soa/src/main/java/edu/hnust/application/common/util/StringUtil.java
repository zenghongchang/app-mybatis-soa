...
import edu.hnust.application.common.util.LinkedStack;
import edu.hnust.application.common.util.LogicUtil;
...
public final class StringUtil {
        ...
        for (int i = 0, len = str.length(); i < len; i++) {
            ...
            if (Character.isHighSurrogate(c)) {// 如果已经是高代理字符，则可能是超过\uFFFF的unicode了
                ...
                if (codePoint == c) {// 解析后的值与单个字符想通，说明只有单个高代理字符，则编码有问题，需要过滤该字符
                    continue;
                } else if (!StringUtil.isXMLCharacter(codePoint)) {// 非法xml字符滤掉
            ...
            if (!StringUtil.isXMLCharacter(c)) {// 跳过非法xml字符
     ...
     * 
     * @param targetStrBuf @param subStr @throws
     */
     ...
     * 
     * @param str 字符串
     * @return 为null、空、连续空格为false，反之为true，
     */
